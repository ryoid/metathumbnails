...
import playgroundTabs, { Tabs } from "../Playground";
import previewTabs from "../Playground/PreviewTabs";
...
const LiveSatori = withLive(function ({
  ...
  const [options, setOptions] = useState<any | object | null>(null);
...
function ResetCode({ activeCard }: { activeCard: string }) {
  ...
  return (
    <button
      onClick={() => {
        (editedCards as any)[activeCard] = playgroundTabs[activeCard];
        onChange(editedCards[activeCard]!);
